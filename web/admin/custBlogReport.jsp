...
<%
    ...
    ResultSet rs0;
    String sql0 = "SELECT `customer_id`,`partner_first_name`, `partner_last_name` FROM `customer` WHERE CONCAT(`first_name`, ' ', `last_name`)= '"+customer_name+"'";
    rs0 = getCon().createStatement().executeQuery(sql0);
    while(rs0.next()){
        partners_name = rs0.getString("partner_first_name")+" "+rs0.getString("partner_last_name");
        custID= rs0.getString("customer_id");
    }
    ...
    try {
        ...
        Paragraph Oadd0 = new Paragraph("Customer Name: "+customer_name, FontFactory.getFont(FontFactory.HELVETICA, 11, new CMYKColor(100, 100, 0, 89)));
        document.add(Oadd0);
        
        Paragraph Oadd1 = new Paragraph("Partner's Name: "+partners_name, FontFactory.getFont(FontFactory.HELVETICA, 11, new CMYKColor(100, 100, 0, 89)));
        document.add(Oadd1);
        document.add(new Phrase("---------------------------------------------------------------------------------------------"));
...
//    document.add(someSectionText);
        ...
        ResultSet rs;
        String query = "SELECT * FROM `couple_blog` WHERE `customer_id` = '"+custID+"'";
